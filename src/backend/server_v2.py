...
async def get_dashboard_stats(db=Depends(get_db)):
    ...
    try:
        async with db.acquire() as conn:
            ...
            stats = stats or {}
            return {
                "organizations": stats.get('organizations', 0) or 0,
                "assessments": stats.get('assessments', 0) or 0,
                "open_risks": stats.get('open_risks', 0) or 0,
                "frameworks": stats.get('frameworks', 0) or 0,
                "avg_compliance": round(stats.get('avg_compliance', 0) or 0, 2),
                "risk_distribution": [dict(r) for r in risk_dist] if risk_dist else []
            }
