...
class FeatureFlagManager:
    ...
    def _load_flags(self):
        ...
        try:
            if self.config_path.exists():
                ...
                # Handle both formats: direct flags object or flags array
                if 'flags' in config_data:
                    # Array format: {"flags": [...]}
                    for flag_data in config_data['flags']:
                        flag = self._parse_flag(flag_data)
                        self.flags[flag.name] = flag
                else:
                    # Direct object format: {"flag_name": {...}, ...}
    ...
    def get_flag_status(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            gradual_flags = sum(
                1 for flag in self.flags.values() 
                if flag.rollout.strategy == RolloutStrategy.GRADUAL
            )
            full_flags = sum(
                1 for flag in self.flags.values() 
                if flag.rollout.strategy == RolloutStrategy.FULL
